...
int search(int depth, U32 curPiece[16], int curPie[14], int alpha, int beta);//搜尋最佳走步 
...
void ai2()
{
	...
	chess(piece, 0);
	search(0, piece, piece_count, -999999, 999999);
...
int search(int depth, U32 curPiece[16], int curPie[14], int alpha, int beta)
{
	...
	if (tAEMi > 0) {
		for (int i = 0; i < tAEMi; i++)
		{
			...
			weight[wp][2] = search(depth + 1, tempPiece, curPie, alpha, beta);
			...
			if (curPiece[15] & (1 << ssrc) && ch & (1 << ssrc) && depth <= noReDepth) { //若為未翻子 在未翻子的遮罩內 depth<=3 
				...
				for (int pID = 0; pID < 14; pID++) { //搜尋可能會翻出之子
					if (DCount[pID]) { //若該兵種可能被翻出
						...
						weight[wp][0] = ssrc;
						weight[wp][1] = ssrc;
						weight[wp][2] += ((DCount[pID] + 1) * search(depth + 1, tempPiece, curPie, alpha, beta));
						DCount[pID]++;
						//將模擬翻出的子復原
					}
				}
...
void dynamicPower()
{
	piPw[0] = 600 + piece_count[7] * 600 + piece_count[8] * 270 + piece_count[9] * 90 + piece_count[10] * 40 + piece_count[11] * 15 + piece_count[12] * 200;
	piPw[1] = 270 + piece_count[8] * 270 + piece_count[9] * 90 + piece_count[10] * 40 + piece_count[11] * 15 + piece_count[12] * 200 + piece_count[13] * 10;
	piPw[2] = 90 + piece_count[9] * 90 + piece_count[10] * 40 + piece_count[11] * 15 + piece_count[12] * 200 + piece_count[13] * 10;
	piPw[3] = 40 + piece_count[10] * 40 + piece_count[11] * 15 + piece_count[12] * 200 + piece_count[13] * 10;
	piPw[4] = 15 + piece_count[11] * 15 + piece_count[12] * 200 + piece_count[13] * 10;
	piPw[5] = 200 + piece_count[7] * 600 + piece_count[8] * 270 + piece_count[9] * 90 + piece_count[10] * 40 + piece_count[11] * 15 + piece_count[12] * 200 + piece_count[13] * 10;
	piPw[6] = 10 + piece_count[7] * 600 + piece_count[13] * 10;

	piPw[7] = 600 + piece_count[0] * 600 + piece_count[1] * 270 + piece_count[2] * 90 + piece_count[3] * 40 + piece_count[4] * 15 + piece_count[5] * 200;//24300
	piPw[8] = 270 + piece_count[1] * 270 + piece_count[2] * 90 + piece_count[3] * 40 + piece_count[4] * 15 + piece_count[5] * 200 + piece_count[6] * 10;//15500
	piPw[9] = 90 + piece_count[2] * 90 + piece_count[3] * 40 + piece_count[4] * 15 + piece_count[5] * 200 + piece_count[6] * 10;//8300
	piPw[10] = 40 + piece_count[3] * 40 + piece_count[4] * 15 + piece_count[5] * 200 + piece_count[6] * 10;//6000
	piPw[11] = 15 + piece_count[4] * 15 + piece_count[5] * 200 + piece_count[6] * 10;//4950
	piPw[12] = 200 + piece_count[0] * 600 + piece_count[1] * 270 + piece_count[2] * 90 + piece_count[3] * 40 + piece_count[4] * 15 + piece_count[5] * 200 + piece_count[6] * 10;//20800
	piPw[13] = 10 + piece_count[0] * 600 + piece_count[6] * 10;//6600
}
