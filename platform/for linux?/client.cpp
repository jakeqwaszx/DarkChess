...
void printwinrecord() {
	...
	foutrc << totalturn << " ply cycle \n" << " ## Win : " << lose << " ,   Lose : " << win << " ,  Draw : " << draw << " ,  PlayTime : " << totalturn - turn + 1 << "  ##\n";
...
bool doenter() {
	...
	START = clock();
	if (player) {
		system("./search2");// excute enterroom's search.exe
		...
		if (!first) {
			if (board[i].length() >= 10 && board[i].at(2) != 't' && board[i].length() <= 12) {// plus move made by enterroom's search.exe at the col only have one move
				if (move[0].at(0) == '1') {// flip
					...
					cout << "?????"<<showboard[move[2].at(0) - 'a'][move[2].at(1) - '1']<<endl;
					if (showboard[move[2].at(0) - 'a'][move[2].at(1) - '1'] == 'X') {// eat blank
						win++;
	...
	for (; turn > 0; turn--) {
		...
		while (1) {
			if (!first) {// first:creatroom , second:enterroom
				...
				if (docreate(buffer)) { err++; printwinrecord(); break; }
