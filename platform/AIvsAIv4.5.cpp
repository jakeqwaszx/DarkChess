...
string winrecorderr = formattime + " err.txt";
...
void printwinrecord(int state) {
	...
	ofstream foutrce(winrecorderr, ios::app);
	if (state == 0) {
		...
		foutrce << totalturn << " ply cycle \n" <<" createroom err: "<<CrErr<<" enterroom err: "<<EnErr<< " ,  PlayTime : " << totalturn - turn + 1 << "  ##\n";
	}
	foutrc.close();
	foutrcm.close();
	foutrce.close();
...
bool repetition() {
	...
	for (int i = 0; i < size - 7; i++) {
		for (int j = i; j < size - 7; j++) {
			if (recordmove[i].substr(recordmove[i].length() - 11, 2) == recordmove[j].substr(recordmove[j].length() - 8, 2)) {
				tworepeat1=1 ;
			}
		}
		for (int j = i; j < size - 7; j++) {
			if (recordmove[i].substr(recordmove[i].length() - 5, 2) == recordmove[j].substr(recordmove[j].length() - 2, 2)) {
				tworepeat2 = 1;
			}
		}
	}
	...
	for (; turn > 0; turn--) {
		...
		while (1) {
			if (!first) {// first:creatroom , second:enterroom
				...
				if (docreate()) { CrErr++; printwinrecord(2); break; }
				if (check()) { break; }
				//system("pause");
				cout << "enter step: " << step << endl;
				if (doenter()) { EnErr++;  printwinrecord(2); break; }
				printwinrecord(2);
				if (check()) { break; }
				if (repetition()) { break; }
				//system("pause");
