...
#include <math.h>
#include <windows.h>
//目前版本2.0 使用pmove2遮罩減少搜尋時間 先使用移動搜尋8層有棋未翻可空過 結果更糟或相同則翻 
//翻子搜尋只到depth=2 depth3以上只搜移動或吃子 算分 棋力 - depth

//可強化 可翻棋減少 搜尋更深? 可翻棋較少則增加翻棋數? 
...
U32 CGen(int ssrc,U32 toccupied)
{
	...
	U32 result=0;
	U32 resulta=0;
	U32 x = ( (rank[r] & toccupied) ^ (1<<ssrc) ) >> (4*r);//取出跟炮同列 削去炮本身那格
	if(x&&c==0){
		result|=CGenCL(x);
	}
	else if(x&&c==1){
...
{
	chess(curBlack,curRed,curPiece,depth);
	int weight[200][3];//計算所有移動與翻棋的得分0src 1dst 2weight 
	int wp=0;
	int eat=0;//先吃 
	U32 taEM[1000][2];//存可吃子的方法 0 src 1 dst
	int tAEMi=AEMindex;//alleatmove index
	...
	if(tAEMi+tAOMi!=0)
	{
		...
		for(int i=0;i<tAEMi;i++)
		{
			...
			weight[wp][0] =taEM[i][0];
			weight[wp][1] =taEM[i][1];
			weight[wp][2] =1+searchMove(deeper,curPiece,curRed,curBlack,curOccupied,curPie,alpha,beta);
			UnSimMove(taEM[i][0],taEM[i][1],curPiece,&curRed,&curBlack,&curOccupied,curPie,copy);
			if(depth%2==0)//max
			{
				if(weight[wp][2]>alpha)
	...
	if(wp==1) best=countAva(curPie,depth);
	return best;
}
