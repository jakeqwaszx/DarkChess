...
void ai3()
{
	...
	*/
	...
	for(int curi=1;curi<100000;curi++)//跑幾輪 
	{
		...
		while(!currentParent->leaf)
		{
			...
			for(int i=0;i<currentParent->child.size();i++)
			{
				...
				else UCB=0.7 * ((currentParent->child[i]->win / currentParent->child[i]->round) + c * sqrt(log(currentParent->round) / currentParent->child[i]->round)) + 0.3 * currentParent->child[i]->MinmaxScore;
		...
		while(ancestor!=NULL)
		{
			ancestor->win += curWincount;
			ancestor->round += simRound;
	...
	for(int i=0 ; i<FcurNode->child.size();i++)
	{
		...
		cout<<FcurNode->child[i]->MinmaxScore<<" "<<FcurNode->child[i]->src<<" "<<FcurNode->child[i]->dst<<" "<<score<<" "<<FcurNode->child[i]->win<<" "<<FcurNode->child[i]->round<<endl;
		if(chosen<score)
		{
			chosen=score;
...
double MCSsimulation2(node *SimNode)
{
	...
	while(true)
	{
		...
		if(curSimNode->depth - drawdepth >= 40) return 0.5;
		//-------------------------------------------------------能吃先吃 吃最大子 
		int bestway=-1;
		int besteat=-1;
