	...
	for(int curi=1;curi<30000;curi++)//跑幾輪 
	{
		...
		while(!currentParent->leaf)
		{
			...
			if(bestway==-1)
			{
				...
				if(currentParent->dst == -1)//翻棋
				{
					currentParent = currentParent->parent;
				}
				if(currentParent->src == -1)//全部算完 
				{
					dieway = 1;
					break;
				}
		...
		for(int i=0;i<AEMindex;i++)
		{
			...
			if(curNode->red & curNode->piece[15] == 0) curNode->endgame=true;
			else if(curNode->black & curNode->piece[15] == 0) curNode->endgame=true;
			curNode->parent->child.push_back(curNode);
			//nodes[curNode->depth].push_back(curNode);
		}
		for(int i=0;i<AOMindex;i++)
