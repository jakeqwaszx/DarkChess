	...
	for(int curi=1;curi<30000;curi++)//跑幾輪 
	{
		...
		while(!currentParent->leaf)
		{
			...
			for(int i=0;i<currentParent->child.size();i++)
			{
				...
				else UCB = ((currentParent->child[i]->win / currentParent->child[i]->round) + c * sqrt(log(currentParent->round) / currentParent->child[i]->round)) + (Cp * currentParent->child[i]->h) / currentParent->child[i]->round;
