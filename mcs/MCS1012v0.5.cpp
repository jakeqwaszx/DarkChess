...
{
	chess(black,red,piece,0);
	stdScore=countAva(piece_count,0,piece);//test 目前使用現在盤面分數
	if(piece[15]==0xffffffff)
	{
		IndexToBoard(9,9);
		return;
	...
	*/
	...
	for(int curi=1;curi<20000;curi++)//跑幾輪 
	{
		...
		while(!currentParent->leaf)
		{
			...
			{
				double UCB;
				if(currentParent->child[i]->round == 0) UCB=100000+rand()%10000;
		...
		//simulation-----------------------------------------------------------------
		currentParent->win += MCSsimulation2(currentParent);
		currentParent->round++;
...
{
	int PW[300][2];
	int eaten=0;//紀錄發生吃子的depth 超過40沒吃子或翻棋則平手 
	...
	while(true)
	{
		...
		if(curSimNode->piece[1]&curSimNode->piece[2]&curSimNode->piece[3]&curSimNode->piece[4]&curSimNode->piece[5]&curSimNode->piece[6]&curSimNode->piece[7]&curSimNode->piece[15]==0)
		{//紅敗 
			if(color == 0) return 0; 
			else return 1; 
		}
		if(curSimNode->piece[8]&curSimNode->piece[9]&curSimNode->piece[10]&curSimNode->piece[11]&curSimNode->piece[12]&curSimNode->piece[13]&curSimNode->piece[14]&curSimNode->piece[15]==0)
		{//黑敗 
			if(color == 1) return 0;
			else return 1; 
		}
...
int MCSsimulation(int curSimRound,node *SimNode)
{
	...
	while(curSimRound)
	{
		...
		{
			chess(curSimNode->black,curSimNode->red,curSimNode->piece,curDeep);//depth
			vector <vector<U32 > >curAllMove;//存所有走步 
			vector<U32 >temp;
			...
			int rnd;
			if(AEMindex>0) rnd=(rand()%AEMindex)+curchMask.size(); //能吃先吃 
			else rnd=rand()%curAllMove.size();
		...
		/*if(nowScore >= stdScore)
			allWin+=sqrt(nowScore-stdScore);
		else
			allWin-=sqrt(stdScore-nowScore);*/
		if(nowScore >= stdScore)
		{
			allWin++;
		}
