...
U32 MS1B(U32 x)
{ // Most Significant 1 Bit (LS1B)函式
	x |= x >> 32;
	x |= x >> 16;
	x |= x >> 8;
	x |= x >> 4;
...
void chess(U32 tblack, U32 tred, U32 tpiece[16], int deep)
{
	...
	{ //紅
		...
		for (int i = 1; i < 8; i++)
		{					   //1~7 為帥~兵,src 為棋子起點,dest 為終點。  先算我方
...
int search(int depth, U32 curPiece[16], U32 curRed, U32 curBlack, U32 curOccupied, int curPie[14], int alpha, int beta)
{
	...
	if (curPiece[15] != 0) //先試翻棋 做完後call search
	{
		...
		{ //搜尋盤面上 32 個位置
			if (curPiece[15] & (1 << ssrc) && ch & (1 << ssrc) && depth <= noReDepth)
			{ //若為未翻子 在未翻子的遮罩內 depth<=3
				if (depth == 0)
				{
					int r = rand() % 6;
					;
				...
				{ //搜尋可能會翻出之子
					...
					{ //若該兵種可能被翻出
						...
						currentChild->realcount = ((DCount[pID] + 1) * search(deeper, curPiece, curRed, curBlack, curOccupied, curPie, alpha, beta));
...
void readBoard() //讀檔模式 讀取board.txt專用
{
	...
	while (getline(file, str))
	{
		...
		if (str.at(2) == 't')
		{ //start after * time 900
			startmove = 1;
		}
		if (str.at(2) == 'C' && str.at(3) == 'o')
		{ //stop before * Comment 0 0
			startmove = 0;
		}
		if (startmove == 1)
		{
	...
	{ //record in char board[4][8] 處理history 產生現在棋版
		if (move[i].at(2) == '(')
		{ //record move 'flip' (kgmrncp)
