			...
			if(curPiece[15] & ( 1 << ssrc ) && ch & ( 1 << ssrc )&&depth<=noReDepth){ //若為未翻子 在未翻子的遮罩內 depth<=3 
				...
				for(int pID=0; pID<14; pID++){ //搜尋可能會翻出之子
					if(DCount[pID]){ //若該兵種可能被翻出
						...
						weight[wp][1] =ssrc;
												if(depth==0)lock=0;
						if((depth==0&&ssrc==25)||lock==1){
							lock=1;
							for(int j=0;j<16;j++){
								cout<<"curpiece= " << hex << " " << curPiece[j] << dec << endl;
							}
							cout<<" ssrc:"<<ssrc<<" depth:"<<depth<<endl;
							system("pause");
						}
						int tempa=search(deeper,curPiece,curRed,curBlack,curOccupied,curPie,alpha,beta);
						weight[wp][2] += ((DCount[pID])*tempa);
						//test5.pop_back();
						ccc=0;
						if(pID<8)
						{
	...
	else//無吃子移動 
	{
		...
		curOccupied|=c2;//c2有子 
			...
			if(lock==1){
				...
				cout<<" taemi:"<<tAEMi<<" src:"<<taEM[i][0]<<" dst:"<<taEM[i][1]<<" depth:"<<depth<<endl;
				system("pause");
			}
			//test5.pop_back();
		...
		*/
		...
		curOccupied|=c2;//c2有子 
			...
			if(lock==1){
				...
				cout<<" tAOMi:"<<tAOMi<<" src:"<<taEM[i][0]<<" dst:"<<taEM[i][1]<<" depth:"<<depth<<endl;
				system("pause");
			}
			//test5.pop_back();
			//if(taEM[i][1]==13||taEM[i][1]==14||taEM[i][1]==17||taEM[i][1]==18) weight[wp][2];//移動到 13 14 17 18時+1; 
