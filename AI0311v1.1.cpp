...
#include <math.h>
#include <windows.h>
//目前版本1.1 使用pmove2遮罩減少搜尋時間 翻子搜尋只到depth=3 depth3以上只搜移動或吃子 算分 棋力+可吃子*100或移動-敵方可吃子*100或移動 
...
int initial();//初始化 
int chess(U32 tblack,U32 tred,U32 tpiece[16],int deep);//尋找可用移動
...
int ai2()
{
	...
	int nowbest=countAva(piece_count,0);
	int movebest=0;
	if((AEMindex+AOMindex)!=0)
		movebest=searchMove(0,piece,red,black,occupied,piece_count,-999999,999999);//先看走10步會不會更糟 
	if(nowbest>=movebest||movebest==0)
		search(0,piece,red,black,occupied,piece_count,-999999,999999);
	IndexToBoard(srci,dsti);
}

int chess(U32 tblack,U32 tred,U32 tpiece[16],int deep)
{
	U32 dest;//可以吃子的行動
	AEMindex=0;
	AOMindex=0;
...
int search(int depth,U32 curPiece[16],U32 curRed,U32 curBlack,U32 curOccupied,int curPie[14],int alpha,int beta)
{
	...
	if((depth==maxDepth||(depth>=3&&etaemi==0))&&depth%2==0)
	{
		return countAva(curPie,depth);
	}
	else if(depth==maxDepth||(depth>=3&&etaemi==0))
	{
		return -countAva(curPie,depth);
	}
	long weight[200][3];//計算所有移動與翻棋的得分0src 1dst 2weight 
	int wp=0;
	int bestsrc;
	int bestdst;
	int count=0;
	if(curPiece[15]!=0)//先試翻棋 做完後call search 
	{
		for(int ssrc=0; ssrc<32; ssrc++){ //搜尋盤面上 32 個位置
			if(curPiece[15] & ( 1 << ssrc ) && ch & ( 1 << ssrc )&&depth<=3){ //若為未翻子 在未翻子的遮罩內 depth<=3 
				if(depth==0)
				{
					int r=rand()%6;;
				...
				for(int pID=0; pID<14; pID++){ //搜尋可能會翻出之子
					if(DCount[pID]){ //若該兵種可能被翻出
						...
						weight[wp][0] =ssrc;
						weight[wp][1] =ssrc;
						weight[wp][2] += -((DCount[pID]+1)*search(deeper,curPiece,curRed,curBlack,curOccupied,curPie,-beta,-alpha));
	...
	if(tAEMi+tAOMi!=0)//再試移動 
	{
		...
		for(int i=0;i<tAEMi;i++)
		{
			...
			//cout<<deeper;
			weight[wp][2] =-search(deeper,curPiece,curRed,curBlack,curOccupied,curPie,-beta,-alpha);
	...
	for(int i=0;i<wp;i++)
	{
		...
		if(weight[i][2]>best||weight[i][2]==-9999999)
		{
			bestsrc=weight[i][0];
			bestdst=weight[i][1];
			best=weight[i][2];
...
int searchMove(int depth,U32 curPiece[16],U32 curRed,U32 curBlack,U32 curOccupied,int curPie[14],int alpha,int beta)
{
	...
	if((depth==maxDepth||tAEMi+tAOMi==0)&&depth%2==0)
	{
		return countAva(curPie,depth);
	}
	else if(depth==maxDepth||tAEMi+tAOMi==0)
	{
		return -countAva(curPie,depth);
	}
	long weight[200][3];//計算所有移動與翻棋的得分0src 1dst 2weight 
	...
	if(tAEMi+tAOMi!=0)//再試移動 
	{
		...
		for(int i=0;i<tAEMi;i++)
		{
			...
			weight[wp][0] =taEM[i][0];
			weight[wp][1] =taEM[i][1];
			weight[wp][2] =-searchMove(deeper,curPiece,curRed,curBlack,curOccupied,curPie,-beta,-alpha);
			UnSimMove(taEM[i][0],taEM[i][1],curPiece,&curRed,&curBlack,&curOccupied,curPie,copy);
			if(weight[wp][2]>alpha)
			{
				alpha=weight[wp][2];
