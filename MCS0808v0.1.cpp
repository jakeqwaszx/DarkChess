...
{
	chess(black,red,piece,0);
	stdScore=countAva(piece_count,0,piece);
	...
	for(int curi=1;curi<=1;curi++)//跑幾輪 
	{
		...
		while(currentParent->child.size())
		{
			...
			{
				int UCB=(currentParent->child[i]->win / currentParent->child[i]->round) + c * sqrt(log10(allRound) / currentParent->child[i]->round);
				if(currentParent->child[i]->depth % 2==1&&currentParent->child[i]->deeper)
				{
					if(UCB>bestUCB)
					{
						bestUCB=UCB;
		...
		cout<<AEMindex<<" a"<<endl;
		vector <vector<U32 > >curAllMove;//存所有走步 先吃子 再走步 再翻棋 
		curAllMove.resize(50);
		int allMovePtr=0;
		for(int i=0;i<AEMindex;i++)
		{
			curAllMove[allMovePtr].push_back(allEatMove[i][0]);//src
			curAllMove[allMovePtr].push_back(allEatMove[i][1]);//dst
			allMovePtr++;
		}
		for(int i=0;i<AOMindex;i++)
		{
			curAllMove[allMovePtr].push_back(allOnlyMove[i][0]);//src
			curAllMove[allMovePtr].push_back(allOnlyMove[i][1]);//dst
			allMovePtr++;
		}
		for(int i=0;i<chMask.size();i++)
		{
			curAllMove[allMovePtr].push_back(chMask[i]);//src
			curAllMove[allMovePtr].push_back(chMask[i]);//dst
			allMovePtr++;
		}
		//simulation-----------------------------------------------------------------
		for(int i=0;i<allMovePtr;i++)
		{
			//模擬走步 
	...
	for(int i=0 ; i<nodes[1].size();i++)
	{
		...
		cout<<" "<<nodes[1][i]->src<<" "<<nodes[1][i]->dst<<"	"<<score<<endl;
		if(chosen<score)
		{
			chosen=score;
			srci=nodes[1][i]->src;
...
int MCSsimulation(int curSimRound,node *SimNode)
{
	...
	while(curSimRound)
	{
		...
		for(int deepi = 0; deepi < simDepth; deepi++)//往下20層 
		{
			...
			{
				curAllMove[allMovePtr].push_back(curchMask[i]);//src
				curAllMove[allMovePtr].push_back(curchMask[i]);//dst
				allMovePtr++;
			}
			for(int i=0;i<AEMindex;i++)
			{
			...
			if(allMovePtr==0) continue;//無棋可翻或走 
			int rnd=rand()%allMovePtr;
		...
		chess(curSimNode->black,curSimNode->red,curSimNode->piece,curSimNode->depth+1);
		int nowScore=countAva(curSimNode->piece_count,curSimNode->depth+1,curSimNode->piece);
		if(nowScore > stdScore) allWin++;
		curSimRound--;//直到0 
	}
	return allWin;
}
