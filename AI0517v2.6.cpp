...
//阻止平手 該子全部-999999 有計算movepoint(可吃或被吃加減分)
//更新:debug 勝負加減分 最後吃或被吃算分完成 
...
int countAva(int pie[14],int deep,U32 curPiece[16])//將士相車馬炮兵
{
	...
	if(color==0)//紅 
	{
		...
		if(redleft==0) power=-10000000+deep*100000;
		if(blackleft==0) power=10000000-deep*100000;
	}
	else
	{
		for(int i=7;i<14;i++)
	...
	if(deep%2==0)
	{
		...
		movePoint=PointMayEat+AOMindex-EAEMindex*20-EAOMindex;
	} 
	else
	{
		int check=0;//如果沒找到誰被吃 
		...
		movePoint=EAEMindex*10+EAOMindex-PointMayEat-AOMindex;
	} 
