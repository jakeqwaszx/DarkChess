...
int countAva(int pie[14],int deep);//呼叫則傳回當前棋版
...
int findPiece(int place);//傳編號 回傳在這個編號的棋子 
using namespace std;
...
0x12100000,0x25200000,0x4A400000,0x84800000,0x21000000,0x52000000,0xA4000000,0x48000000};//棋子移動遮罩 炮 
U32 pMoves2[32]={
...
0x16110000,0x2D220000,0x4B440000,0x86880000,0x61100000,0xD2200000,0xB4400000,0x68800000};//翻棋遮罩 十字 
U32 pMoves3[32]={
...
0x36310000,0x7D720000,0xEBE40000,0xC6C80000,0x63100000,0xD7200000,0xBE400000,0x6C800000};//翻棋遮罩 十字加左上 左下 右上 右下 
...
string current_position[32];//盤面狀況總覽 未實作 
string history;
int timeCount;//剩餘時間 未實作 
...
int eaten=0;//可能發生的吃子情況 
int srci,dsti;//index版 0~31
int oneway=0;//1時啟用單向搜尋 
int maxDepth=4;
int moveMaxDepth=8;
...
void ai2()
{
	...
	//onewaySearch(0,piece,red,black,occupied,piece_count,-999999,999999,0);
...
int countAva(int pie[14],int deep)//將士相車馬炮兵
{
	...
	{
		eat[i]=findPiece(EallEatMove[i][1]);//把下一步會被吃得棋子存入eat 0~13
	...
	return power+movePoint-deep-biggest;
}
...
int onewaySearch(int depth,U32 curPiece[16],U32 curRed,U32 curBlack,U32 curOccupied,int curPie[14],int alpha,int beta,int eat)
{
	chess(curBlack,curRed,curPiece,depth);
	int weight[100][3];//計算所有移動與翻棋的得分0src 1dst 2weight 
	int wp=0; 
	...
	if(tAEMi+tAOMi!=0)
	{
		...
		for(int i=0;i<tAEMi;i++)
		{
			...
			weight[wp][0] =taEM[i][0];
			weight[wp][1] =taEM[i][1];
			weight[wp][2] =onewaySearch(deeper,curPiece,curRed,curBlack,curOccupied,curPie,alpha,beta,1);
			UnSimMove(taEM[i][0],taEM[i][1],curPiece,&curRed,&curBlack,&curOccupied,curPie,copy);
			if(depth%2==0)//max
			{
				if(weight[wp][2]>alpha)
		...
		for(int i=0;i<tAOMi;i++)
		{
			...
			weight[wp][0] =taOM[i][0];
			weight[wp][1] =taOM[i][1];
			weight[wp][2] =onewaySearch(deeper,curPiece,curRed,curBlack,curOccupied,curPie,alpha,beta,0);
			UnSimMove(taOM[i][0],taOM[i][1],curPiece,&curRed,&curBlack,&curOccupied,curPie,copy);
			if(depth%2==0)//max
			{
				if(weight[wp][2]>alpha)
