...
using namespace std;
//同型表 先無翻棋移動 分數低於初始盤面再翻棋(因為翻棋不能剪也不能用同型表很慢)
					...
					if (curPiece[15] & (1 << ssrc) && ch & (1 << ssrc) && depth <= noReDepth) { //若為未翻子 在未翻子的遮罩內 depth<=3 
						...
						for (int pID = 0; pID < 14; pID++) { //搜尋可能會翻出之子
							if (DCount[pID]) { //若該兵種可能被翻出
								...
								weight[wp][0] = ssrc;
								weight[wp][1] = ssrc;
								weight[wp][2] += ((DCount[pID] + 1) * search(depth + 1, tempPiece, curPie, alpha, beta, 1, hashvalue ^ randtable[pID][ssrc] ^ randtable[pID][ssrc]));
								DCount[pID]++;
								//將模擬翻出的子復原
							}
						}
