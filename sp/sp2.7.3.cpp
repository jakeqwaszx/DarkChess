...
using namespace std;
//同型表 翻棋後不能用 原始版 分數同newsp2.6 比newsp2.6慢
...
int main()
{
	...
	drawOrNot();
	dynamicPower();
	/*int onboardfi = 0;//計算場面上未翻棋數量 
	for (int i = 0; i < 14; i++)
	{
		onboardfi += DCount[i];
	}
	int onboardmi = 0;//計算場面上剩餘(明加暗)棋數量 
	for (int i = 0; i < 14; i++) {
		onboardmi += piece_count[i];
	}
	if (onboardfi == 0 && onboardmi < 5)maxDepth = 30;
	else if (onboardfi == 0&& onboardmi<9)maxDepth = 19;
	else if(onboardfi == 0)maxDepth = 11;
	else if (onboardfi < 25)maxDepth = 5;*/
			...
			if (curPiece[15] & (1 << ssrc) && ch & (1 << ssrc) && depth <= noReDepth) { //若為未翻子 在未翻子的遮罩內 depth<=3 
				...
				for (int pID = 0; pID < 14; pID++) { //搜尋可能會翻出之子
					if (DCount[pID]) { //若該兵種可能被翻出
						...
						weight[wp][0] = ssrc;
						weight[wp][1] = ssrc;
						weight[wp][2] += ((DCount[pID] + 1) * search(depth + 1, tempPiece, curPie, alpha, beta, 1, hashvalue ^ randtable[pID][ssrc] ^ randtable[14][ssrc] ));
						DCount[pID]++;
						//將模擬翻出的子復原
					}
				}
...
/*
	if (depth == 0)//max
	{
		...
		for (int i = wp - 1; i >= 0; i--)
		{
			...
			if (weight[i][2] == best)
			{
				recordis.push_back(i);
				srcis.push_back(weight[i][0]);
				dstis.push_back(weight[i][1]);
			}
		}
		int index = rand() % recordis.size();
		recordi = recordis[index];
		srci = srcis[index];
		dsti = dstis[index];
*/
	if (depth == 0)//max
