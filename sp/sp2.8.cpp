					...
					for(int ii = i + 7;ii < 15; ii++)
					{
						...
						if(curPiece[ii])
						{
							...
							while(tempcpb)
							{
								...
								int y=ia%4;
								x+=y;//x為兩者距離
								int tempS=influenceValue[i][ii-7];
								if(x==1) tempS /= 2;
								else
								{
					...
					for(int ii = i - 7;ii < 8; ii++)
					{
						...
						if(curPiece[ii])
						{
							...
							while(tempcpb)
							{
								...
								int y=ia%4;
								x+=y;//x為兩者距離
								int tempS=influenceValue[i-7][ii];
								if(x==1) tempS /= 2;
								else
								{
									tempS = tempS * pow(2 ,2-x );
			...
			if (curPiece[15] & (1 << ssrc) && ch & (1 << ssrc) && depth <= noReDepth) { //若為未翻子 在未翻子的遮罩內 depth<=3 
				...
				for (int pID = 0; pID < 14; pID++) { //搜尋可能會翻出之子
					if (DCount[pID]) { //若該兵種可能被翻出
						...
						weight[wp][0] = ssrc;
						weight[wp][1] = ssrc;
						weight[wp][2] += ((DCount[pID] + 1) * searchnf(depth + 1, tempPiece, curPie, hashvalue ^ randtable[pID][ssrc] ^ randtable[14][ssrc]));
						DCount[pID]++;
						//將模擬翻出的子復原
					}
				}
...
int searchnf(int depth, U32 curPiece[16], int curPie[14], U32 hashvalue)
{
	...
	{
		int re = countAva(curPie, depth, curPiece);
		hashtablef[hashindex].depth = depth;
		hashtablef[hashindex].count = re;
		memcpy(hashtablef[hashindex].curPiece, curPiece, sizeof(hashtablef[hashindex].curPiece));
		return re;
	}
	int weight[100][3];//計算所有移動與翻棋的得分0src 1dst 2weight 
	int wp = 0;
	...
	if (tAEMi > 0) {
		for (int i = 0; i < tAEMi; i++)
		{
			...
			weight[wp][0] = taEM[i][0];
			weight[wp][1] = taEM[i][1];
			weight[wp][2] = searchnf(depth + 1, tempPiece, curPie, hashvalue ^ randtable[c1p - 1][GetIndex(c1)] ^ randtable[c2p - 1][GetIndex(c2)] ^ randtable[c1p - 1][GetIndex(c2)]);
			curPie[c2p - 1]++;
			if (depth % 2 == 0)//max
			{
				if (weight[wp][2] > best)
	...
	for (int i = 0; i < tAOMi; i++)//純移動 
	{
		...
		weight[wp][2] = searchnf(depth + 1, tempPiece, curPie, hashvalue ^ randtable[c1p - 1][GetIndex(c1)] ^ randtable[c1p - 1][GetIndex(c2)]);
	...
	hashtablef[hashindex].depth = depth;
	hashtablef[hashindex].count = best;
	memcpy(hashtablef[hashindex].curPiece, curPiece, sizeof(hashtablef[hashindex].curPiece));
	return best;
}
void drawOrNot()
