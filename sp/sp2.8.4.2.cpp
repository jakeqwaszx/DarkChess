...
double search(int depth, U32 curPiece[16], int curPie[14], double alpha, double beta, U32 hashvalue);//搜尋最佳走步 
...
struct hashdata {
	double count; int depth; U32 curPiece[16];
};
...
double countAva(int pie[14], int deep, U32 curPiece[16])//將士相車馬炮兵
{
	...
	if (color == 0)//紅 
	{
		for (int i = 0; i < 7; i++)
		{
			power += (double)pie[i] * (double)piPw[i];
			redleft += pie[i];
		}
		for (int i = 7; i < 14; i++)
		{
		...
		if (redleft == 0) power = -100000.0 + (double)deep * 1000;
		if (blackleft == 0) power = 100000.0 - (double)deep * 1000;
	}
	else
	{
		for (int i = 7; i < 14; i++)
	...
	if (curPiece[15] != 0) {
		...
		movePoint /= 7248;//負最大 ee7200+eo248
	}
	else {
		if (deep % 2 == 0)
		{
		...
		movePoint /= 11768;//負最大 ee11520+eo248
	}
	//------------------------------------------------------加總
	return powerw * power + movepointw * movePoint - deepw * deep / maxDepth + distancew * distanceScore;
}

double search(int depth, U32 curPiece[16], int curPie[14], double alpha, double beta, U32 hashvalue)
{
	test++;
	...
	double re = countAva(curPie, depth, curPiece);
	...
	double weight[100][3];//計算所有移動與翻棋的得分0src 1dst 2weight 
	int wp = 0;
	double best = -9999999;
	if (depth % 2 == 1)
		best = 9999999;
			...
			if (curPiece[15] & (1 << ssrc) && ch & (1 << ssrc) && depth <= noReDepth) { //若為未翻子 在未翻子的遮罩內 depth<=noReDepth 
				...
				for (int pID = 0; pID < 14; pID++) { //搜尋可能會翻出之子
					if (DCount[pID]) { //若該兵種可能被翻出
						...
						weight[wp][0] = ssrc;
						weight[wp][1] = ssrc;
						weight[wp][2] += ((DCount[pID] + 1) * search(deeper, curPiece, curPie, -999999, 999999, hashvalue ^ randtable[pID][ssrc] ^ randtable[14][ssrc]));
						ccc = 0;
						if (pID < 8)
						{
							curPiece[cpID] ^= c;
