...
int countAva(int pie[14], int deep, U32 curPiece[16])//將士相車馬炮兵
{
	...
	//-----------------------------------------------------找誰被吃的函數 
	/*int check=0;//如果沒找到誰被吃 找到則是1~14看誰被吃 
	...
	for(int i=0; i<AEMindex ;i++)//i所有被吃子 
	{
		...
		if(color==0 && check < 8) PointMayEat=0-PointMayEat;
		else if(color==1 && check > 7) PointMayEat=0-PointMayEat;
		//cout<<PointMayEat<<" "<<color<<" "<<check<<endl;
		if(check==0)
		{
			//cout<<"err1 ";
		}
