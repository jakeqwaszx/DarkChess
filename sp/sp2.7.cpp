...
int myhash(U32 tpiece[16]);//傳入盤面，傳回hash值
int randint(int n);
...
int search(int depth, U32 curPiece[16], int curPie[14], int alpha, int beta, int flip)
{
	...
	{
		hashtable[hashvalue % hashtable.size()].depth = depth;
		hashtable[hashvalue % hashtable.size()].count = re;
		memcpy(hashtable[hashvalue % hashtable.size()].curPiece, curPiece, sizeof(hashtable[hashvalue % hashtable.size()].curPiece));
		return re;
	}
		...
		for (int i = 0; i < tAEMi; i++)
		{
			...
			if (!flip) {
				if (beta <= alpha)
				{
					if (depth % 2 == 1) {
						hashtable[hashvalue % hashtable.size()].depth = depth;
						hashtable[hashvalue % hashtable.size()].count = beta;
						memcpy(hashtable[hashvalue % hashtable.size()].curPiece, curPiece, sizeof(hashtable[hashvalue % hashtable.size()].curPiece));
						return beta;
					}
					else {
						hashtable[hashvalue % hashtable.size()].depth = depth;
						hashtable[hashvalue % hashtable.size()].count = alpha;
						memcpy(hashtable[hashvalue % hashtable.size()].curPiece, curPiece, sizeof(hashtable[hashvalue % hashtable.size()].curPiece));
						return alpha;
					}
				}
	...
	hashtable[hashvalue % hashtable.size()].depth = depth;
	hashtable[hashvalue % hashtable.size()].count = best;
	memcpy(hashtable[hashvalue % hashtable.size()].curPiece, curPiece, sizeof(hashtable[hashvalue % hashtable.size()].curPiece));
	return best;
}
