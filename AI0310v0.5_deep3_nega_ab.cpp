...
int ai()
{
	chess(black,red,piece,0);//找出所有敵我行動 
...
int countAva(int pie[14],int deep)//將士相車馬炮兵 
{
	...
	int movePoint=0;
	if(deep%2==0)
		;//movePoint=AEMindex+AOMindex-EAEMindex-EAOMindex;
	else
		;//movePoint=EAEMindex+EAOMindex-AEMindex-AOMindex;
...
int search(int depth,U32 curPiece[16],U32 curRed,U32 curBlack,U32 curOccupied,int curPie[14],int alpha,int beta)
{
				...
				for(int pID=0; pID<14; pID++){ //搜尋可能會翻出之子
					if(DCount[pID]){ //若該兵種可能被翻出
						...
						if(pID<8)
						{
							...
							curPiece[15]^=c;
							DCount[pID]--;
							int gamma=-alpha;//alpha beta加上負號互換 
							alpha=-beta;
							beta=gamma;
						}
						else
	...
	if(1)//max 原本depth%2==0 nega max不需要 
	{
		for(int i=0;i<wp;i++)
		{
			if(depth==0)
